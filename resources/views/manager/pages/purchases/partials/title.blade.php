<div class="card mb-3 p-2">
    ...
    <div class="d-flex justify-content-between">
        ...
        <div>
            @if(manager()->can('purchasing-print'))
            ...
            <script type="text/javascript">
                function openPrintReceipt(id) {
                    window.open(
                        "{{ URL::to('manager/purchases/print/purchase/') }}/" + id ,
                        "_blank",
            ...
            @if($purchase->status == 'Pending')
                @if(manager()->can('purchasing-edit'))
                    ...
                    @livewire('manager.pages.purchases.partials.edit', ['purchase' => $purchase], key('purchase-edit-'.time()))
                @endif  
                @if(manager()->can('purchasing-convert'))
                    <a class="btn btn-sm btn-outline-primary" href="javascript:void(0);"
                        wire:click.prevent="$dispatch('purchaseConvert', { id: {{ $purchase->id }} })"
                    >
                        <i class='bx bx-repost'></i>
                    </a>
                    @livewire('manager.pages.purchases.partials.convert-purchase', ['purchase' => $purchase], key('purchase-convert-'.time()))
                @endif  
                @if(manager()->can('purchasing-save'))
                    <a class="btn btn-sm btn-outline-primary" href="javascript:void(0);"
                        wire:click.prevent="$dispatch('purchaseSave', { id: {{ $purchase->id }} })"
                    >
                        <i class='bx bx-save'></i>
                    </a>
                    @livewire('manager.pages.purchases.partials.save', ['purchase' => $purchase], key('purchase-save-'.time()))
                @endif  
                @if(manager()->can('purchasing-delete'))
                    <a class="btn btn-sm btn-danger" href="javascript:void(0);"
                        wire:click.prevent="$dispatch('purchaseDelete', { id: {{ $purchase->id }} })"
                    >
                        <i class="bx bx-trash me-1"></i>
                    </a>
                    @livewire('manager.pages.purchases.partials.delete', ['purchase' => $purchase], key('purchase-delete-'.time()))
                @endif  
            @endif
        </div>
    </div>
