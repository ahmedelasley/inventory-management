        ...
        <div>
            ...
            <script type="text/javascript">
                function openPrintReceipt(id) {
                    window.open(
                        "{{ URL::to('manager/orders/print/order/') }}/" + id ,
                        "_blank",
            ...
            @if($order->status == 'Open')
                @if($order->type == 'Pending')
                    @if(manager()->can('transfer-edit'))
                        ...
                        @livewire('manager.pages.orders.partials.edit', ['order' => $order], key('order-edit-'.time()))
                    @endif

                    @if(manager()->can('transfer-send'))
                        <a class="btn btn-sm btn-outline-primary" href="javascript:void(0);"
                            wire:click.prevent="$dispatch('orderSend', { id: {{ $order->id }} })"
                        >
                            <i class='bx bx-send'></i>
                        </a>
                        @livewire('manager.pages.orders.partials.send', ['order' => $order], key('order-send-'.time()))
                    @endif
                ...
                @endif
                    ...
                    @livewire('manager.pages.orders.partials.convert-order', ['order' => $order], key('order-convert-'.time())) --}}

                @if($order->type == 'Send')
                    @if(manager()->can('transfer-process'))
                        <a class="btn btn-sm btn-outline-primary" href="javascript:void(0);"
                            wire:click.prevent="$dispatch('orderProcessed', { id: {{ $order->id }} })"
                        >
                            <i class='bx bx-loader-circle'></i>
                        </a>
                        @livewire('manager.pages.orders.partials.processed', ['order' => $order], key('order-processed-'.time()))
                    @endif
                    
                @endif
                @if($order->type == 'Processed')
                    @if(manager()->can('transfer-shipped'))
                        ...
                        @livewire('manager.pages.orders.partials.shipped', ['order' => $order], key('order-shipped-'.time()))
                    @endif
                @endif

                @if($order->type == 'Shipped')
                    @if(manager()->can('transfer-save'))
                        <a class="btn btn-sm btn-outline-primary" href="javascript:void(0);"
                            wire:click.prevent="$dispatch('orderSave', { id: {{ $order->id }} })"
                        >
                            <i class='bx bx-save'></i>
                        </a>
                        @livewire('manager.pages.orders.partials.save', ['order' => $order], key('order-save-'.time()))
                    @endif
                @endif
            @endif
            @if($order->status == 'Open' && $order->type == 'Pending')
                @if(manager()->can('transfer-delete'))
                    ...
                    @livewire('manager.pages.orders.partials.delete', ['order' => $order], key('order-delete-'.time()))
