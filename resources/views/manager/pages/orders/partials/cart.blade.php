<div class="card">
    @livewire('manager.pages.orders.partials.edit-product', ['order' => $order])
    @livewire('manager.pages.orders.partials.delete-product', ['order' => $order])
    ...
    <div class="card-body">
        @if(manager()->can('transfer-all-items'))
                        ...
                        <tr  wire:key="product-{{$value->id}}" class="{{ $value->stock?->quantity < $value->quantity_request ? 'table-danger' : '' }}">
                            ...
                            @if($order->status == 'Open' && ( $order->type == 'Pending' || $order->type == 'Processed' ))
                                ...
                                @if($order->type == 'Pending' || $order->type == 'Processed' )  
                                    @if(manager()->can('transfer-edit-qty-request-item') || manager()->can('transfer-edit-qty-send-item'))
                                ...
                                @endif
                                @if( $order->type == 'Pending')  
                                    @if(manager()->can('transfer-delete-item'))
