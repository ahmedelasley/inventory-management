        ...
        <div class="card mb-2 ">
            ...
            <div class="card-title d-flex justify-content-between flex-wrap">
                ...
                <select wire:model.live="paginate" class="form-control w-10 mt-3 me-3" style="width:75px" id="paginate">
                    ...
                    <option value="{{ getSetting('pagination') }}">{{ getSetting('pagination') }} sys</option>
            ...
            <div class="card-body">
                    ...
                    <table class="table table-striped table-hover table-sm text-center">
                        ...
                        <tbody class="table-border-bottom-0">
                            @forelse ($data as $value)
                                ...
                                <tr wire:key="record-{{ $value->id }}">
                                    <td>{{$loop->iteration }}</td>
                                    <td><strong>{{ $value->name }}</strong></td>
                                    <td>{{ $value->email }}</td>
                                    <td><span class="badge bg-label-primary">{{ $value->email_verified_at ? 'Active' : '' }}</span></td>
                                    <td><span class="badge bg-label-warning">{{ $value->kitchen?->name }}</span></td>
                                    <td>
                ...
                <div class="demo-inline-spacing">
                    <nav aria-label="d-flex">
                        <ul class="pagination-sm justify-content-end">
                            {{ $data->links() }}
                        </ul>
                    </nav>
                </div>
