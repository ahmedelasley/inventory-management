<li class="nav-item navbar-dropdown dropdown-notification dropdown mx-3" >
    <a class="nav-link dropdown-toggle hide-arrow" href="javascript:void(0);" data-bs-toggle="dropdown">
      <div class="avatar avatar-online">
        ...
        @if(Auth::guard('keeper')->user()->unreadNotifications->count())
        <span class="flex-shrink-0 badge badge-center rounded-pill bg-danger w-px-20 h-px-20" style="margin-left: -20px;">{{ Auth::guard('keeper')->user()->unreadNotifications->count() }}</span>
        @endif
      </div>
    </a>
    @if(Auth::guard('keeper')->user()->unreadNotifications->count())
    <ul class="dropdown-menu dropdown-menu-end " style="width: 300px;">
      <li class="mb-2 ">
        <div class="dropdown-header d-flex justify-content-between align-items-center">
          ...
          <a href="{{ route('keeper.notifications.read.all') }}" class="text-primary"><i class='bx bxs-show'></i></a>
        </div>
        <div class="dropdown-divider"></div>

      </li>
      <div style="width: 300px;height : calc(100vh - 700px);position: relative;overflow: auto;">
        @foreach(Auth::guard('keeper')->user()->unreadNotifications->take(5) as $notification)
          <li class="dropdown-item p-2 my-0" >
            <a class="" href="{{ route('keeper.orders.create.order', ['order' => $notification->data['details']['order_id'], "notification_id" => $notification->id ] ) }}" >
              {{-- <div class="avatar">
      ...
      <li class="dropdown-item text-center">
        <a class="" href="{{ route('keeper.notifications.index') }}">Show All Notifications</a>
      </li>                 
    </ul>
    @else
    <ul class="dropdown-menu dropdown-menu-end " style="width: 300px;">
