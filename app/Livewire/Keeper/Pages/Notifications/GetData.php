...
class GetData extends Component
{
    ...
    {
        // $data = Kitchen::with(['keeper', 'creator', 'updater'])->where($this->field, 'like', '%' . $this->search . '%')->latest()->paginate($this->paginate);
        $data = Auth::guard('keeper')->user()->notifications()->where("data->details->$this->field", 'like', '%' . $this->search . '%')->latest()->paginate(20);
        return view('keeper.pages.notifications.get-data', [
            'data' => $data,
        ]);
    }
}
