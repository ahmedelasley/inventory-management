...
class ConvertOrder extends Component
{
    ...
    public function submit()
    {
        ...
        try {
            ...
            $details = [
                ...
                'title' => "'{$this->order->type}' '{$this->order->code}' from keeperistrator",
            ...
            // Notify the user
            foreach (Keeper::where('id', '!=', Auth::guard('keeper')->user()->id)->get() as $keeper) {
                $keeper->notify(new TrackOrderStatus($details));
            }
    ...
    {
        return view('keeper.pages.orders.partials.convert-order');
    }
}
