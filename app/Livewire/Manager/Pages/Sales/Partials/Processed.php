...
use App\Livewire\Manager\Pages\Sales\GetData;
...
class Processed extends Component
{
    ...
    protected $listeners = ['saleProcessed'];

    public function saleProcessed($id)
    {
        ...
        // Open modal
        $this->dispatch('processedSaleModalToggle');
    }
    ...
    public function submit()
    {
        ...
        try {
            ...
            $saleStatus->new_status = 'Processed';
            $saleStatus->date = now();
            $saleStatus->statusable()->associate($service);
            $saleStatus->save();

            // 1- Update sale
            $this->sale->type   = 'Processed';
...
// dd($product->stock->product_id);
    ...
    {
        return view('manager.pages.sales.partials.processed');
    }
}
